...
import com.example.tongxiang.gesture.MyGestureDector;
import com.example.tongxiang.gesture.MyScaleGestureDetector;
import com.example.tongxiang.model.ChinaMapModel;
import com.example.tongxiang.model.ProvinceModel;
import com.example.tongxiang.util.ChinaMapSvgUtil;
...
public class MapView  extends View {
        private Paint innerPaint,outerPaint,mPaint;//画省份的内部画笔和外圈画笔
        ...
        public void setOnProvinceClickLisener(MapView.onProvinceClickLisener onProvinceClickLisener) {
    ...
    private void drawOneArea(Canvas pCanvas, Paint textPaint, int index) {
        ...
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("屠甸镇")) {
            nameX=900;
            nameY=850;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("石门镇")) {
            nameX=380;
            nameY=650;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("濮院镇")) {
            nameX=850;
            nameY=420;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("梧桐街道")) {
            nameX=650;
            nameY=570;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("河山镇")) {
            nameX=175;
            nameY=600;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("州泉镇")) {
            nameX=100;
            nameY=800;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("高桥镇")) {
            nameX=670;
            nameY=950;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("大麻镇")) {
            nameX=80;
            nameY=1050;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("凤鸣街道")) {
            nameX=500;
            nameY=780;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("崇福镇")) {
            nameX=370;
            nameY=950;
        }
        if (chinaMapModel.getProvinceslist().get(index).getName().contains("乌镇镇")) {
            nameX=500;
            nameY=400;
        }
        pCanvas.drawText(chinaMapModel.getProvinceslist().get(index).getName(),nameX,nameY,textPaint);
        ...
        private void drawMap(Canvas canvas) {
            ...
            if (chinaMapModel.getProvinceslist().size()>0){
                ...
                if (selectPosition!=-1){
                    ...
                    innerPaint.setColor(chinaMapModel.getProvinceslist().get(selectPosition).getrightColor());
                    outerPaint.setColor(chinaMapModel.getProvinceslist().get(selectPosition).getSelectBordercolor());
                    outerPaint.setStrokeWidth(2.5f);
                    mPaint.setColor(chinaMapModel.getProvinceslist().get(selectPosition).getSelectNameColor());
                    mPaint.setTextSize(50);
                    for (Path p:chinaMapModel.getProvinceslist().get(selectPosition).getListpath()){
                        canvas.drawPath(p,  innerPaint);
                        canvas.drawPath(p,  outerPaint);
                    }
                    drawOneArea(canvas,mPaint,selectPosition);
